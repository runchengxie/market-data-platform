...
`marketdata migration status` 会区分已在平台内实现的原生工作流和仍由相邻仓库提供实现的过渡后端。当前 `hk-depth` 与 `hk-assets` 都是原生工作流，没有过渡后端。
...
（资产、metadata、intraday cache、release、HK health/audit 报告），研究 runs、sweeps、
