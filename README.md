...
`DATA_PLATFORM_ROOT` 是本仓库推荐使用的统一环境变量。`HK_DATA_PLATFORM_ROOT` 仍作为旧 HK 调用方的兼容变量保留。只有当某个项目明确需要将运行结果、缓存或报告也集中放到该根目录下时，才需要使用 `CSTREE_ARTIFACTS_ROOT` 环境变量。通常情况下，各策略仓库应保持独立输出目录，并统一通过 `DATA_PLATFORM_ROOT` 读取已发布市场数据。
